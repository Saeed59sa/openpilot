...
class DesireHelper:
    ...
    def update(
        ...
        if (
            not (frogpilot_toggles.lane_detection and one_blinker)
            or below_lane_change_speed
        ):
            lane_available = True
        else:
            desired_lane = (
        ...
        lanes_valid = (
            frogpilotPlan.laneWidthLeft > 2.5 and frogpilotPlan.laneWidthRight > 2.5
        )

        aalc_trigger = (
            frogpilot_toggles.aalc_enabled
            and frogpilotPlan.aalcActive
            and lane_available
            and lead_distance < 30
            and blindspot_clear
            and speed_ok
            and no_manual
            and lanes_valid
        ...
        if not lateral_active or self.lane_change_timer > LANE_CHANGE_TIME_MAX:
            ...
            one_blinker
            and below_lane_change_speed
            and not carstate.standstill
            and frogpilot_toggles.use_turn_desires
        ):
            self.lane_change_state = LaneChangeState.off
            self.lane_change_direction = LaneChangeDirection.none
            self.turn_direction = (
