...
class CruiseButtonHandler:
  ...
  def _update_button_state(self, btn_events):
...
class CruiseController:
  def __init__(self, CP, CI):
    ...
    self.curve_speed_clu = V_CRUISE_UNSET
  ...
  def _calculate_max_speed(self, CS, sm, cluster_speed: float, v_cruise_kph: float):
    ...
    if road_limit_speed is not None and self.min_set_speed_clu <= road_limit_speed < current_max_speed_clu:
      ...
      speed_bp = [self.conv.to_current_unit(10.0), self.conv.to_current_unit(100.0)]
  ...
  def _calculate_curve_speed(self, model_msg, current_speed_ms: float, v_cruise_kph: float):
    ...
    final_curve_speed_ms = min(calculated_curve_speed_ms, self.conv.to_ms(v_cruise_kph))
    self.curve_speed_clu = self.conv.to_clu(final_curve_speed_ms)
  ...
  def _calculate_steer_based_speed(self, current_speed_ms: float, steering_angle_deg: float) -> float:
    ...
    if abs(steering_angle_deg) >= steering_decel_angle_deg * 2:
      steer_based_speed_ms = max(min(current_speed_ms * 0.70, current_speed_ms - 3.0), self.conv.to_ms(V_CRUISE_MIN))
      return self.conv.to_clu(steer_based_speed_ms)
    elif abs(steering_angle_deg) >= steering_decel_angle_deg:
      steer_based_speed_ms = max(min(current_speed_ms * 0.85, current_speed_ms - 3.0), self.conv.to_ms(V_CRUISE_MIN))
      return self.conv.to_clu(steer_based_speed_ms)
  ...
  def update_v_cruise(self, CS, sm, enabled: bool):
    ...
    if CS.cruiseState.enabled:
      if not self.long_control or not self.CP.pcmCruise:
        ...
        if CS.cruiseState.speed == 0 or CS.cruiseState.speed == -1:
  ...
  def _update_cruise_button(self, v_cruise_kph, btn_events, enabled):
    if enabled:
      btn, long_pressed = self.btn_handler._update_button_state(btn_events)
      self.btn_handler.btn_long_pressed = long_pressed
...
class CruiseStateManager:
  def __init__(self):
    ...
    self._disable_cruise_and_reset_speed()

    self.prev_brake_pressed = False
    self.prev_main_button_state = False
  ...
  def _disable_cruise_and_reset_speed(self):
    self.enabled = False
    self.speed = self.conv.to_ms(V_CRUISE_UNSET)

  def update(self, CS, main_buttons):
    btn, long_pressed = self.btn_handler._update_button_state(CS.buttonEvents)
    if btn != ButtonType.unknown:
      self._handle_button_press(CS, btn, long_pressed)

    self._handle_main_button_toggle(main_buttons[-1])
    self._update_state_based_on_conditions(CS)
  ...
  def _handle_button_press(self, CS, btn, long_pressed):
    v_cruise_delta = 10 if self.conv.is_metric else IMPERIAL_INCREMENT * 5
    road_limit_speed_nda = SpeedLimiter.instance().get_road_limit_speed()
    road_limit_speed_stock = CS.exState.navLimitSpeed
    ...
    current_cruise_clu = int(round(self.conv.to_clu(self.speed)))
    v_cruise_kph = current_cruise_clu
  ...
  def _handle_main_button_toggle(self, current_main_button_state: bool) -> None:
    if current_main_button_state != self.prev_main_button_state and current_main_button_state:
      self.available = not self.available
    self.prev_main_button_state = current_main_button_state

  def _update_state_based_on_conditions(self, CS) -> None:
