...
class CruiseStateManager:
  ...
  def update(self, CS, main_buttons):
    btn = self._update_buttons(CS)
    if btn != ButtonType.unknown:
      self._handle_button_press(CS, btn)
  ...
  def _handle_button_press(self, CS, btn):
    ...
    current_cruise_clu = int(round(self.conv_to_clu(self.speed)))
